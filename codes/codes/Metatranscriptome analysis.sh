item=yourItem
# pear
~/pear/pear-0.9.10-linux-x86_64/bin/pear -f ~/projects/$item/$item`echo _`1.fastq  -r  ~/projects/$item/$item`echo _`2.fastq -o ~/projects/$item/$item -j 128
rm ~/projects/$item/$item`echo _`1.fastq
rm ~/projects/$item/$item`echo _`2.fastq
fi

# trimmomatic
java -jar ~/Trimmomatic/Trimmomatic-0.36/trimmomatic-0.36.jar SE -phred33 -threads 128 ~/projects/$item/$item.assembled.fastq ~/projects/$item/$item.Trimmomatic.fastq  SLIDINGWINDOW:4:15 MINLEN:99

# sortmerna
~/miniconda3/pkgs/sortmerna-4.2/bin/sortmerna --ref ~/sortmerna_db/smr_v4.3_default_db.fasta \
        --reads ~/projects/$item/$item.Trimmomatic.fastq \
        --aligned ~/projects/$item/aligned \
        --other ~/projects/$item/filtered \
        --fastx -threads 128 -v \
        --workdir temp_/ 

#diamond
~/diamond/diamond blastx --db ~/diamond_db/RefSeq_bac -q ~/projects/$item/filtered.fastq -a ~/projects/$item/$item`echo _`output -t ./ -k 1 -p 128

~/diamond/diamond view --daa ~/projects/$item/$item`echo _`output.daa -o ~/projects/$item/$item`echo _`final -f tab
...
if "-O" not in sys.argv:
	if "-F" not in sys.argv:
		if "-R" not in sys.argv:
			sys.exit("WARNING1: need to specify either organism results (with -O flag in command), reference IDs (with -R flag in command), or functional results (with -F flag in command).")
